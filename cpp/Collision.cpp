...
Collision::ContactManifold Collision::getEdgeContactManifold(Polyhedron* A, Polyhedron* B, EdgeDirectionQuery* query, IsStatic isStatic){
  ...
  if(isStatic == IsStatic::A){
    ...
    contactManifold.points.emplace_back(ContactPoint{query->separation, getNearestPointOnLineB(*(query->edgeA->startVertex),
                                                                                               *(query->edgeA->twin->startVertex),
                                                                                               *(query->edgeB->startVertex),
                                                                                               *(query->edgeB->twin->startVertex))});
    ...
    contactManifold.points.emplace_back(ContactPoint{query->separation, getNearestPointOnLineA(*(query->edgeA->startVertex),
                                                                                               *(query->edgeA->twin->startVertex),
                                                                                               *(query->edgeB->startVertex),
                                                                                               *(query->edgeB->twin->startVertex))});
  }
    ...
    contactManifold.points.emplace_back(ContactPoint{query->separation, getNearestPointBetweenLines(*(query->edgeA->startVertex),
                                                                                                    *(query->edgeA->twin->startVertex),
                                                                                                    *(query->edgeB->startVertex),
                                                                                                    *(query->edgeB->twin->startVertex))});
  }
...
Collision::ContactManifold Collision::getSphereBoxContactManifold(glm::vec3 positionA, float radiusA, glm::vec3 halfWidth, glm::mat4 modelMatrixB, glm::mat4 modelMatrixInverseB){
  ...
  if(offsetLength < radiusA){ // sphere overlaps the box
    ...
    if(offsetLength <= 0.0f){ // sphere center is inside the box
      float distances[6];
      glm::vec3 normals[6];
      distances[0] = std::abs(halfWidth.x - sphereCenter.x); normals[0] = glm::vec3(1.0f, 0.0f, 0.0f);
      distances[1] = std::abs(-halfWidth.x - sphereCenter.x); normals[1] = glm::vec3(-1.0f, 0.0f, 0.0f);
      distances[2] = std::abs(halfWidth.y - sphereCenter.y); normals[2] = glm::vec3(0.0f, 1.0f, 0.0f);
      distances[3] = std::abs(-halfWidth.y - sphereCenter.y); normals[3] = glm::vec3(0.0f, -1.0f, 0.0f);
      distances[4] = std::abs(halfWidth.z - sphereCenter.z); normals[4] = glm::vec3(0.0f, 0.0f, 1.0f);
      distances[5] = std::abs(-halfWidth.z - sphereCenter.z); normals[5] = glm::vec3(0.0f, 0.0f, -1.0f);
