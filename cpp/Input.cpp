...
void Input::update(sf::RenderWindow* renderWindow){
  ...
  if(sf::Mouse::isButtonPressed(sf::Mouse::Left)) MouseDown[0] = true;
  else MouseDown[0] = false;
  testMouseHit(0);

  if(sf::Mouse::isButtonPressed(sf::Mouse::Middle)) MouseDown[1] = true;
  else MouseDown[1] = false;
  testMouseHit(1);

  if(sf::Mouse::isButtonPressed(sf::Mouse::Right)) MouseDown[2] = true;
  else MouseDown[2] = false;
  testMouseHit(2);
...
bool Input::keyHit(std::string keybind, PlayerKeybindProfiles* profile, Input::Joystick* joystick){
  ...
  if(windowActive){
    if(profile->kbm != nullptr && profile->kbm->keybindings.find(keybind) != profile->kbm->keybindings.end()) kbmHit = profile->kbm->keybindings.at(keybind).hit();
    if(profile->controller != nullptr && profile->controller->keybindings.find(keybind) != profile->controller->keybindings.end() && joystick != nullptr) controllerHit = profile->controller->keybindings.at(keybind).hit(joystick->number);
...
bool Input::keyDown(std::string keybind, PlayerKeybindProfiles* profile, Input::Joystick* joystick){
  ...
  if(windowActive){
    if(profile->kbm != nullptr && profile->kbm->keybindings.find(keybind) != profile->kbm->keybindings.end()){
      kbmDown = profile->kbm->keybindings.at(keybind).down();
    }
    if(profile->controller != nullptr && profile->controller->keybindings.find(keybind) != profile->controller->keybindings.end() && joystick != nullptr){
